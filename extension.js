...
function activate(context) {
    ...
    const lunchFlutterAppCommand = vscode.commands.registerCommand(
        'jenny-vscode-extension.launchFlutterApp', function () {
            panel = createWebviewPanel(context);
        },
    );

    const sendMessageToFlutter = vscode.commands.registerCommand('jenny-vscode-extension.sendMessageToFlutter', async () => {
        if (!panel.webview) {
...
function createWebviewPanel(context) {
    const webBuildPath = path.join(context.extensionPath, 'jenny_app', 'build', 'web');
    const webAssetsPath = path.join(webBuildPath, 'assets');

    const panel = vscode.window.createWebviewPanel(
        'jennyApp', // Unique identifier for this type of panel
        'Jenny App', // Title displayed in the tab
